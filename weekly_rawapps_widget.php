...
//  Adding Action
add_action('plugins_loaded','weekly_display_widget');

//  Display Widget
function weekly_display_widget() {
    register_sidebar_widget(__('Top Weekly RawApps Android Widget'),'weekly_rawapps_widget');
    register_widget_control('Top Weekly RawApps Android Widget','weekly_widget_options');

}
//  Creating RawApps  widget
function weekly_rawapps_widget($args) {
    extract($args);
    $options=get_option("weekly_rawapps_widget");
    if (!is_array($options)) {
	    $options=array(
    ...
    weekly_rawapps_widget_template();
    echo $after_widget;
   ...
   //   Widget template
    function weekly_rawapps_widget_template() {include_once(WP_PLUGIN_DIR."/android-market-top-weekly-apps/rawapps_widget_template.php");}

   //   Widget Control Panel
    function weekly_widget_options()
    {
	...
	if ($_POST['weekly-rawapps-widget-submit']) {
	    ...
	    update_option("weekly_rawapps_widget",$options);
	}
	//Form
	...
	echo "<input type='hidden' id='weekly-rawapps-widget-submit' name='weekly-rawapps-widget-submit' value='1' />";
